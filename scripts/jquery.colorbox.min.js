...
function jGalleryViewer(imgs) {
   var self = this;
   var currentUid = 0;
   var tmpl = '<div><div id="cbox${id}" class="block" style="left:${left}"><table style="width:100%;height:100%;vertical-align:middle;text-align:center;border-collapse:collapse;"><tr><td><div><img src="" draggable="false"></div><div id="cboxComment"></div></td></tr></table></div></div>';
   ...
   var index = 0,
      ...
      isClose = false,  // or vertically (close), but not both
		isClosing = false; // are we going to close the viewer soon?
   ...
   function loadImg(div, index) {
      ...
      div.find('img').attr('src', href).unbind('load').load(function() {
         isLoaded[index] = true;
   ...
   function startDrag(e) {
		if(isClosing) // We already decided to close the viewer, ignore subsequent touches
			return;

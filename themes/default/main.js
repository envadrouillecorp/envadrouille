...
var defaultTheme = {
    ...
    _month:['', 'Jan', 'Fev', 'Mar', 'Avr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
    ...
    // TODO: clean + create var dirs[] to avoid modifying json
    showDirs:function(data, all, div, height) {
        if(!data.dirs)
            return;
        ...
        for (var i in data.dirs) {
            if(i >= 4 && !defaultTheme.default_show_all && !all)
                break;
            ...
            if(!data.dirs[i].isSearchResult) {
                ...
                if(m) {  
                    ...
                    if(previousYear === null)
                        previousYear = m[1];
                    if(previousYear != m[1] && (all || defaultTheme.default_show_all)) {
        ...
        if(data.dirs.length > 4 && !defaultTheme.default_show_all && !all) {
        ...
        if(!div) {
            ...
            if(data.dirs.length > 4 && !defaultTheme.default_show_all && !all) {
                $('#show').animate({opacity:1}, 'fast').click({json:data}, function(e) {
                    defaultTheme.default_show_all = true;
                    defaultTheme.showDirs(e.data.json, true);
                });
            }
        }
    },
    ...
    showSearch:function(dataFull, dataPartial, regs) {
        if(dataFull.length) {
            ...
            defaultTheme.showDirs({dirs:defaultTheme.showSearchResults(dataFull, regs)}, true, 'search_full', 1);
        }
        if(dataPartial.length) {
            ...
            defaultTheme.showDirs({dirs:defaultTheme.showSearchResults(dataPartial, regs)}, true, 'search_part', 1);
        }
    },
    ...
    showContent:function(content, json) {
        switch(content) {
            case 'dirs': return defaultTheme.showDirs(json, jGallery.currentPage != '');
            case 'pics': return defaultTheme.showPics(json);
